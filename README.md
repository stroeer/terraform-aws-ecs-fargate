...
| <a name="input_health_check_grace_period"></a> [health\_check\_grace\_period](#input\_health\_check\_grace\_period) | Set the start-up grace period with which to ignore health checks. This can be important to for containers with long startup times. E.g. Windows containers. | `number` | `0` | no |
